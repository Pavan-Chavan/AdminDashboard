  ...
  try {
    ...
    for (const marketType of marketTypes) {
      ...
      const processBatch = async (batch) => {
        await Promise.all(batch.map(async (option) => {
          try {
      ...
      const batchSize = process.env.BAJARBHAV_BATCH_SIZE; // Define the batch size
      for (let i = 0; i < marketTypeData.DropdownOptions.length; i += batchSize) {
        const batch = marketTypeData.DropdownOptions.slice(i, i + batchSize);
